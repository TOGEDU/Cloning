...
interface RouteParams {
  ...
  onRecordComplete: (item: {id: number; text: string}) => void;
}

type RecordingScreenRouteProp = RouteProp<{params: RouteParams}, 'params'>;
...
const RecordingScreen: React.FC = () => {
  ...
  const [recordingState, setRecordingState] = useState<
    'idle' | 'recording' | 'paused' | 'listening'
  >('idle'); // 상태 'idle', 'recording', 'paused', 'listening'
  ...
  }, [route.params?.onRecordComplete, sound, navigation]);
    ...
    try {
      ...
      if (uri) {
        formData.append('voiceRecord', {
          uri: Platform.OS === 'ios' ? uri.replace('file://', '') : uri, // 파일 경로 처리
          type: 'audio/mpeg',
