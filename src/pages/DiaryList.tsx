...
const DiaryList: React.FC = () => {
  ...
  const fetchDiaryDates = async (year: number, month: number) => {
    try {
  ...
  const checkDiaryExistence = async (date: string) => {
    try {
      const token = await AsyncStorage.getItem('authToken');
  ...
  const getMarkedDates = (): MarkedDates => {
    ...
    const customMarkedDates: MarkedDates = {
      ...markedDates,
      [today]: {
        marked: true,
        dotColor: 'red',
    ...
    <View style={styles.container}>
      ...
      <View style={styles.bottomContainer}>
        <Calendar
          ...
          monthFormat={'M월'}
          onMonthChange={(month) => {
            fetchDiaryDates(month.year, month.month);
          }}
